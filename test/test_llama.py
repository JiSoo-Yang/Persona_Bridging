...
# ============================================================================
# Hugging Face 모델 래퍼 (Llama-3.2-1B 전용)
# ============================================================================

class HuggingFaceModelWrapper:
    """
    Llama-3.2-1B 전용 래퍼
    - Gated model: HF token 필요
    - CPU float32
    - Llama-3 chat template 사용
    """
    def __init__(
        self,
        model_name: str,
        ...
        print(f"\n[Llama-3.2] Loading {model_name} on {self.device} (mode={self.mode})")

        # Llama는 gated model이므로 HF token 필요
        ...
        # pad token 설정 (Llama는 eos_token을 pad_token으로 사용)
    ...
    def invoke(self, messages, do_sample: bool = False, temperature: float = 0.0):
        ...
        # 메타 표현 제거
        cleaned = re.sub(r'^\s*<(think|analysis|reasoning)>\s*', '', response_text, flags=re.IGNORECASE)
...
Ask questions one at a time, analyzing each response before asking the next."""
    ...
    for i in range(num_questions):
        ...
        assistant_msg = type('Message', (), {'type': 'assistant', 'content': target_answer})()
        ...
        if i < num_questions - 1:
            feedback_prompt = (
                f"The target answered: '{target_answer}'\n\n"
...
if __name__ == "__main__":
    ...
    if not HF_TOKEN:
        ...
        print("Llama-3.2-1B is a gated model and requires HF token.")
        print("Get your token from: https://huggingface.co/settings/tokens")
        print("Then add to .env file: HF_TOKEN=your-token-here")
        raise SystemExit(1)
